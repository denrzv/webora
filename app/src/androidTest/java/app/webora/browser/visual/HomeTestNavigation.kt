 ...
 *
 * The final Back can replace SiteSkin chrome with native Home asynchronously. Waiting only for the
 * Navigation Hub after every click can therefore block forever on a control that was correctly removed
 * while Home is still composing. Each iteration instead accepts exactly two legal continuation states:
 * native Home, which completes the traversal, or an integrated Navigation Hub, which permits another Back.
 */
